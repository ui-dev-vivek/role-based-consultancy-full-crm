...
<div>
    ...
    <div class="relative">
        <select id="{{ $name }}" name="{{ $name }}" required="{{ $required ? 'required' : '' }}" {{
            $attributes->merge(['class' => 'p-2 block w-full rounded-md border-0 py-2.5 text-slate-900 shadow-sm ring-1
            ring-inset ring-slate-300 focus:ring-2 focus:ring-inset focus:ring-primary-600 sm:text-sm sm:leading-6
